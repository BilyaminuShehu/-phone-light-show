...
function loadState() {
  try {
    ...
    for (const [deviceId, entry] of Object.entries(saved.pool || {})) {
      ...
      // pendingWinMessage used to be stored as a plain string; it's now
      // { message, winnerId } (see 'winner-contact' below) — normalize an
      // old-format string from a state file saved before this change so
      // it doesn't silently deliver "undefined" to a reconnecting winner.
      let pendingWinMessage = entry.pendingWinMessage || null;
      if (typeof pendingWinMessage === 'string') pendingWinMessage = { message: pendingWinMessage, winnerId: null };
      pool.set(deviceId, { ws: null, connectedAt: entry.connectedAt, hasWon: entry.hasWon, pendingWinMessage });
    }
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join') {
      ...
      if (entry.pendingWinMessage) {
        send(ws, { type: 'winner', message: entry.pendingWinMessage.message, winnerId: entry.pendingWinMessage.winnerId });
    ...
    // --- a winner submitting contact info to claim their prize — comes
    // from the participant's own device, not the admin, so this sits
    // before the admin-only gate below. winnerId (see pick-winner above)
    // is how this finds the right winnerHistory entry without needing a
    // heavier per-device auth scheme for what's a low-stakes, opt-in
    // giveaway flow — only the actual winning device was ever sent its
    // winnerId. ---
    if (msg.type === 'winner-contact') {
      ...
      send(ws, { type: 'winner-contact-ok' });
      for (const a of adminSockets) send(a, { type: 'winner-history', history: winnerHistory });
      console.log(`[winner] contact info submitted for winner ${historyEntry.shortId}`);
      markStateDirty();
      flushState(); // rare, high-value event — same reasoning as pick-winner/reset-giveaway
      return;
    }
    ...
    if (msg.type === 'song-cue') {
      // A full, pre-authored light sequence generated by analyzing an
      // uploaded song's actual rhythm client-side (see public/admin.html,
      // analyzeSongForBeats()) — as opposed to 'show-start' above, which
      // only pulses at a constant interval regardless of the music.
      // Reuses the exact same 'cue' broadcast path as the one-off
      // trigger() effects (public/index.html already knows how to run an
      // arbitrary {t, on} pattern and clean up its scheduled timeouts on
      // show-stop), just with a much longer, richer pattern.
      const rawPattern = Array.isArray(msg.pattern) ? msg.pattern : [];
      const MAX_STEPS = 8000; // sanity cap — a 5+ minute song at a step every ~40ms is still well under this
      const cleaned = [];
      let lastT = -1;
      for (const step of rawPattern) {
        if (cleaned.length >= MAX_STEPS) break;
        if (!step || typeof step.t !== 'number' || typeof step.on !== 'boolean') continue;
        if (!Number.isFinite(step.t)) continue;
        const t = Math.max(0, Math.round(step.t));
        if (t < lastT) continue; // defense-in-depth: enforce non-decreasing time order server-side too
        cleaned.push({ t, on: step.on });
        lastT = t;
      }
      if (cleaned.length === 0) {
        send(ws, { type: 'song-cue-error', error: 'No valid pattern steps received — try re-analyzing the song.' });
        return;
      }
      const startAt = Date.now() + LEAD_TIME_MS;
      broadcastToParticipants({ type: 'cue', startAt, pattern: cleaned });
      for (const v of viewerSockets) send(v, { type: 'show-status', live: true });
      // Reuses 'show-start-ack' (not a new message type) — admin.html's
      // existing audio-sync code already listens for this to schedule
      // audio.play() against the same clock-synced instant, no matter
      // which kind of show produced it.
      send(ws, { type: 'show-start-ack', startAt, mode: 'song', intervalMs: 0 });
      console.log(`[admin] song-cue started: ${cleaned.length} steps, ~${Math.round((cleaned[cleaned.length - 1].t) / 1000)}s, at ${new Date(startAt).toISOString()}`);
      return;
    }
    ...
    if (msg.type === 'pick-winner') {
      ...
      // A short random token, not the deviceId itself, identifying THIS
      // specific win. Sent only to the winning device and stored on its
      // own winnerHistory entry — when that device later submits contact
      // info (see 'winner-contact' below), it echoes this back, which is
      // how the server finds the right entry to attach it to without
      // needing a heavier auth mechanism for what's a low-stakes,
      // opt-in giveaway flow.
      const winnerId = crypto.randomUUID();

      const online = entry.ws && entry.ws.readyState === 1;
      if (online) {
        send(entry.ws, { type: 'winner', message, winnerId });
      } else {
        entry.pendingWinMessage = { message, winnerId }; // delivered next time this device reconnects
      }

      const historyEntry = { winnerId, shortId: deviceId.slice(0, 8), message, timestamp: Date.now(), online, contact: null };
